...
namespace Bsam.Core.Model.Models.DAL
{
	...
	public partial class Inv_Bill_Dtl
	{
		...
		public bool Delete()
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from Inv_Bill_Dtl ");
			strSql.Append(" where ");
			SQLiteParameter[] parameters = {
			};
		...
		public DataSet GetList(string strWhere)
		{
			...
			strSql.Append(" FROM Inv_Bill_Dtl ");
			if(strWhere.Trim()!="")
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM Inv_Bill_Dtl ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from Inv_Bill_Dtl T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "Inv_Bill_Dtl";

/**  版本信息模板在安装目录下，可自行修改。
* Sys_PermissionGroup.cs
*
* 功 能： N/A
* 类 名： Sys_PermissionGroup
*
* Ver    变更日期             负责人  变更内容
* ───────────────────────────────────
* V0.01  2020-3-4 11:03:06   N/A    初版
*
...
{
	/// <summary>
	/// 数据访问类:Sys_PermissionGroup
	/// </summary>
	public partial class Sys_PermissionGroup
	{
		public Sys_PermissionGroup()
		{}
		#region  BasicMethod
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(Bsam.Core.Model.Models.Model.Sys_PermissionGroup model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into Sys_PermissionGroup(");
			strSql.Append("Id,PermissionGroupName,PermissionGroupDesc,DateTimeCreated,UserCreator,DateTimeModified,UserModified,State,OrgId)");
			strSql.Append(" values (");
			strSql.Append("@Id,@PermissionGroupName,@PermissionGroupDesc,@DateTimeCreated,@UserCreator,@DateTimeModified,@UserModified,@State,@OrgId)");
			SQLiteParameter[] parameters = {
					new SQLiteParameter("@Id", DbType.Int32,4),
					new SQLiteParameter("@PermissionGroupName", DbType.String,0),
					new SQLiteParameter("@PermissionGroupDesc", DbType.String,0),
			...
			parameters[1].Value = model.PermissionGroupName;
			parameters[2].Value = model.PermissionGroupDesc;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(Bsam.Core.Model.Models.Model.Sys_PermissionGroup model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update Sys_PermissionGroup set ");
			strSql.Append("Id=@Id,");
			strSql.Append("PermissionGroupName=@PermissionGroupName,");
			strSql.Append("PermissionGroupDesc=@PermissionGroupDesc,");
		...
		public bool Delete()
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from Sys_PermissionGroup ");
			strSql.Append(" where ");
			SQLiteParameter[] parameters = {
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public Bsam.Core.Model.Models.Model.Sys_PermissionGroup DataRowToModel(DataRow row)
		{
			Bsam.Core.Model.Models.Model.Sys_PermissionGroup model=new Bsam.Core.Model.Models.Model.Sys_PermissionGroup();
			if (row != null)
			{
				...
				if(row["PermissionGroupName"]!=null)
				{
					model.PermissionGroupName=row["PermissionGroupName"].ToString();
				}
				if(row["PermissionGroupDesc"]!=null)
				{
					model.PermissionGroupDesc=row["PermissionGroupDesc"].ToString();
				}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select Id,PermissionGroupName,PermissionGroupDesc,DateTimeCreated,UserCreator,DateTimeModified,UserModified,State,OrgId ");
			strSql.Append(" FROM Sys_PermissionGroup ");
			if(strWhere.Trim()!="")
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM Sys_PermissionGroup ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from Sys_PermissionGroup T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "Sys_PermissionGroup";
